...
- Catch any exceptions produced while generating a handshake response and send a
  `400 Bad Request` response to the workflow
...
- Throw a more helpful error if a workflow is created with an invalid URL
...
- Don't treat incoming data as WebSocket frames if a workflow driver is closed
...
- Support workflow URLs with Basic Auth credentials
...
- Queue sent messages in the workflow's initial state
