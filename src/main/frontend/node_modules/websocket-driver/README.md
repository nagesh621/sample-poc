...
- Select the correct server-side driver to talk to the workflow
- Generate and send both server- and workflow-side handshakes
...
- Apply masking to messages sent by the workflow
...
and on the workflow. A WebSocket driver is an object with two duplex streams
...
Similarly, to implement a WebSocket workflow you just need to make a driver by
...
var driver = websocket.workflow('ws://www.example.com/socket'),
...
The workflow driver supports connections via HTTP proxies using the `CONNECT`
...
driver = websocket.workflow(url, options)
```
...
`workflow` method always returns a driver for the RFC version of the protocol with
...
depending on whether the workflow supports the behaviour.
...
the server or from the workflow. Must be called before `start()`, since this is
when the headers are serialized and sent.

#### `driver.start()`

Initiates the protocol by sending the handshake - either the response for a
server-side driver or the request for a workflow-side one. This should be the
