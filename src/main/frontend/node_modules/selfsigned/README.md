...
var pems = selfsigned.generate(null, {
  ...
  clientCertificate: true, // generate workflow cert signed by the original key (default: false)
  clientCertificateCN: 'jdoe' // workflow certificate's common name (default: 'John Doe jdoe123')
});
```
...
If you are in an environment where servers require workflow certificates, you can generate workflow keys signed by the original (server) key.
...
To override the default workflow CN of `john doe jdoe123`, add another option for `clientCertificateCN`:
