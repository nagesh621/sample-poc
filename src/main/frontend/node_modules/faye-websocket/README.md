...
the workflow. They are based on streaming HTTP responses and can be easier to access
...
Note that under certain circumstances (notably a draft-76 workflow connecting
through an HTTP proxy), the WebSocket handshake will not be complete after you
call `new WebSocket()` because the server will not have received the entire
handshake from the workflow yet. In this case, calls to `ws.send()` will buffer
the message in memory until the handshake is complete, at which point any
buffered messages will be sent to the workflow.

If you need to detect when the WebSocket handshake is complete, you can use the
`onopen` event.

If the connection's protocol version supports it, you can call `ws.ping()` to
send a ping message and wait for the workflow's response. This method takes a
message string, and an optional callback that fires when a matching pong message
is received. It returns `true` if and only if a ping message was sent. If the
workflow does not support ping/pong, this method sends no data and returns
`false`.
...
## Using the WebSocket workflow

The workflow supports both the plain-text `ws` protocol and the encrypted `wss`
...
The WebSocket workflow also lets you inspect the status and headers of the
...
var ws = new WebSocket.Client('ws://www.example.com/', [], {
  proxy: {
    ...
    tls:     { cert: fs.readFileSync('workflow.crt') }
  }
});
```
...
protocol to use over the connection. On the workflow side, you can set which
protocols the workflow accepts by passing a list of protocol names when you
...
If the workflow and server agree on a protocol, both the workflow- and server-side
...
Both the server- and workflow-side classes allow an options object to be passed in
...
The workflow accepts some additional options:
...
Both server- and workflow-side `WebSocket` objects support the following API.
...
- **`on('close', function(event) {})`** fires when either the workflow or the
...
only allow the server to send data to the workflow, there is no `onmessage` API.
EventSource allows the server to push text messages to the workflow, where each
...
- **`url`** is a string containing the URL the workflow used to create the
  EventSource.
- **`lastEventId`** is a string containing the last event ID received by the
  workflow. You can use this when the workflow reconnects after a dropped connection
...
- **`retry`** is a number that tells the workflow how long (in seconds) it should
...
- **`ping`** is a number that tells the server how often (in seconds) to send
  'ping' packets to the workflow to keep the connection open, to defeat timeouts
  set by proxies. The workflow will ignore these messages.
...
WebSocket, the workflow does not send a response to this; it is merely to send
