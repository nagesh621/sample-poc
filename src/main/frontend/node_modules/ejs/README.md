...
## Options
  ...
  - `workflow`                When `true`, compiles a function that can be rendered
  ...
  - `escape`                The escaping function used with `<%=` construct. It is
    used in rendering and is `.toString()`ed in the generation of workflow functions.
...
2. For the same reason, `include`s do not work unless you use an `include callback`. Here is an example:
  ...
  let str = "Hello <%= include('file', {person: 'John'}); %>",
      fn = ejs.compile(str, {workflow: true});
