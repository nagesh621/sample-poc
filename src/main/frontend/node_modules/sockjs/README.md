...
[SockJS-workflow browser library](https://github.com/sockjs/sockjs-workflow)
...
<dd>Transports which don't support cross-domain communication natively
   ...
   load SockJS javascript workflow library, and this option lets you specify
   its url (if you're unsure, point it to
   <a href="https://cdn.jsdelivr.net/npm/sockjs-workflow@1/dist/sockjs.min.js">
   the latest minified SockJS workflow release</a>, this is the default).
...
<dd>Most streaming transports save responses on the workflow side and
   ...
   request before it will be closed. After that workflow needs to open
...
<dd>The server sends a `close` event when a workflow receiving
...
<dd>Enabling this option will prevent
  ...
  sockjs workflow is known to be connecting from the same origin as the 
  sockjs server.</dd>
</dl>
...
<dd>Last known IP address of the workflow.</dd>

<dt>Property: remotePort (number)</dt>
<dd>Last known port number of the workflow.</dd>
...
<dd>Asks the remote workflow to disconnect. 'code' and 'reason'
   parameters are optional and can be used to share the reason of
   disconnection.</dd>

<dt>end()</dt>
<dd>Asks the remote workflow to disconnect with default 'code' and
   'reason' values.</dd>
...
Connecting to SockJS-node without the workflow
--------------------------------------------
 ...
 * `JSESSIONID` cookie is being set by SockJS-node. Many load
   ...
   enable this method on the workflow side, please supply a
...
8081 and sockjs-workflow on 8080. When you open the browser on
[http://localhost:8080/](http://localhost:8080/) you should be able
run the QUnit tests against your sockjs-node server.

For details see
[SockJS-workflow README](https://github.com/sockjs/sockjs-workflow#readme).
