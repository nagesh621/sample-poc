...
Provides a native javascript workflow and server-side [TLS][] implementation.

__Examples__

```js
// create TLS workflow
var workflow = forge.tls.createConnection({
  server: false,
  ...
  /* provide a workflow-side cert if you want
  getCertificate: function(connection, hint) {
    return myClientCertificate;
  },
  /* the private key for the workflow-side cert if provided */
...
workflow.process(encryptedBytesFromServer);
...
var server = forge.tls.createConnection({
  ...
  cipherSuites: [
    forge.tls.CipherSuites.TLS_RSA_WITH_AES_128_CBC_SHA,
    forge.tls.CipherSuites.TLS_RSA_WITH_AES_256_CBC_SHA],
  // require a workflow-side certificate if you want
  verifyClient: true,
  verify: function(connection, verified, depth, certs) {
    if(depth === 0) {
      var cn = certs[0].subject.getField('CN').value;
      if(cn !== 'the-workflow') {
        verified = {
          alert: forge.tls.Alert.Description.bad_certificate,
          message: 'Certificate common name does not match expected workflow.'
        };
      }
    }
    return verified;
  ...
  connected: function(connection) {
    ...
    connection.prepare(forge.util.encodeUtf8('Hi workflow!'));
    /* NOTE: experimental, start heartbeat retransmission timer
    myHeartbeatTimer = setInterval(function() {
      connection.prepareHeartbeatRequest(forge.util.createBuffer('1234'));
    }, 5*60*1000);*/
  ...
  tlsDataReady: function(connection) {
    // TLS data (encrypted) is ready to be sent to the workflow
    sendToClientSomehow(connection.tlsData.getBytes());
    // if you were communicating with the workflow above you'd do:
    // workflow.process(connection.tlsData.getBytes());
  },
  dataReady: function(connection) {
    // clear data from the workflow is ready
    console.log('the workflow sent: ' +
      forge.util.decodeUtf8(connection.data.getBytes()));
    // close connection
    connection.close();
  },
...
// when encrypted TLS data is received from the workflow, process it
...
socket.on('data', function(data) {
  workflow.process(data.toString('binary')); // encoding should be 'binary'
});
...
Provides a native [JavaScript][] mini-implementation of an http workflow that
uses pooled sockets.
