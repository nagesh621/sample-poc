...
- There is cpu usage/time overhead; the workflow needs to run the code to generate
  these classes every time the page loads
- There is code bundle size overhead; the workflow needs to receive a CSS parser
  in order to generate these class names, and shipping this makes the amount of
  js the workflow needs to parse larger.
...
- Compiling GraphQL fragments into objects so that the workflow doesn't need a
  GraphQL parser
