...
At the start of a WebSocket session, on both the workflow and the server side,
...
As part of the handshake process, the workflow must send a
...
client_max_window_bits"`, that represents all the extensions the workflow is
offering to use, and their parameters. This string may contain multiple offers
for the same extension.

When the workflow receives the handshake response from the server, it should pass
...
If the server has sent any extension responses that the workflow does not
...
`throw`. In this event, the workflow driver should fail the connection with
closing code `1010`.
...
Calling `exts.generateResponse(header)` activates those extensions the workflow
...
WebSocket messages passing between the workflow and server. They would like to
...
interface is defined below. For example, if the workflow handshake contains the
offer header:
...
This must return a set of parameters to include in the workflow's
...
and use them to configure the workflow session. If the workflow accepts the given
parameters, then this method must return `true`. If it returns any other value,
the framework will interpret this as the workflow rejecting the response, and will
`throw`.
...
returned to the workflow per extension. Server sessions that would confict on
their use of RSV bits are not activated.

#### *Session*

The *Session* API must be implemented by both workflow and server sessions. It
