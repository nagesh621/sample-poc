...
You can use browserify on the workflow, or use the pre-compiled script that acts as a polyfill.
...
The example below shows how you can load the promise library (in a way that works on both workflow and server using node or browserify).  It then demonstrates creating a promise from scratch.  You simply call `new Promise(fn)`.  There is a complete specification for what is returned by this method in [Promises/A+](http://promises-aplus.github.com/promises-spec/).
