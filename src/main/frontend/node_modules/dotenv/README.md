...
export const workflow = new Client(process.env.BEST_API_KEY)
```
...
dotenv.config()
errorReporter.workflow.report(new Error('faq example'))
```

`workflow` will not be configured correctly because it was constructed before `dotenv.config()` was executed. There are (at least) 3 ways to make this work.
